...
module.exports = async (client, message) => {
   ...
   let UserData;
    try {
      ...
      if (prefixSchema && prefixSchema.newPrefix !== null) prefix = prefixSchema.newPrefix;
   ...
   const escapeRegex = (str) => str.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    ...
    if(UserData.IsBlacklisted) {
      return // Basically ignores the user who got blacklisted from the bot
    }
    // args: true,
    ...
    if (command.botpermission && !message.guild.me.permissions.has(command.botpermission)) {
        embed.setDescription("I do not have the required permissions for this command")
        return message.channel.send({embeds: [embed]})
    }
